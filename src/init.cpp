...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, hash, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-anonymizehashamount=<n>", strprintf(_("Keep N PIV anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Hash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeHashAmount = GetArg("-anonymizehashamount", 0);
    if (nAnonymizeHashAmount > 999999) nAnonymizeHashAmount = 999999;
    if (nAnonymizeHashAmount < 2) nAnonymizeHashAmount = 2;
    ...
    LogPrintf("Anonymize Hash Amount %d\n", nAnonymizeHashAmount);
