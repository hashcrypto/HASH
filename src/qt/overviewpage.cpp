...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeHashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeHashAmount = strAnonymizeHashAmount.remove(strAnonymizeHashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHashAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeHashAmount * COIN) nMaxToAnonymize = nAnonymizeHashAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeHashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeHashAmount));
        strAnonymizeHashAmount = strAnonymizeHashAmount.remove(strAnonymizeHashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHashAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
